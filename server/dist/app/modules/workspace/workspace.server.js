...
exports.workspaceService = exports.joinWorkspace = exports.deleteWorkspace = exports.updateWorkspace = exports.getSingleWorkspace = exports.getUserWorkspaces = void 0;
...
const getSingleWorkspace = async (workspaceId) => {
    const workspace = await workspace_model_1.Workspace.findById(workspaceId).populate("user");
    if (!workspace) {
...
const joinWorkspace = async (payload, user) => {
    const { email, role, workspaceId } = (0, verifyToken_1.verifyToken)(payload.token);
    if (email !== user?.email) {
    ...
    if (member) {
        throw new AppError_1.default(http_status_1.default.NOT_FOUND, "You are already a member");
    }
    await member_model_1.Member.create({
        role,
        user: user._id,
    ...
    const result = await member_model_1.Member.findOne({
        user: user._id,
        workspace: workspaceId,
    }).populate("workspace");
    return result;
